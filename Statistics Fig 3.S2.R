...
setwd("C:/Users/BESCBEAR/OneDrive - NIOZ")
#setwd("C:/Users/cbeardsworth/OneDrive - NIOZ")


##NIOZ residence patch files
# files_2018 <- list.files(path =paste0("//zeus/cos/birds/bijleveld/fieldwork/WATLAS/residence_patches/2018/islandica/batch1/underlying_data/"), full.names=T)
# files_2019 <- list.files(path =paste0("//zeus/cos/birds/bijleveld/fieldwork/WATLAS/residence_patches/2019/islandica/batch1/underlying_data/"), full.names=T)
# files_2020 <- list.files(path =paste0("//zeus/cos/birds/bijleveld/fieldwork/WATLAS/residence_patches/2020/islandica/batch1/underlying_data/"), full.names=T)
# files_2021 <- list.files(path =paste0("//zeus/cos/birds/bijleveld/fieldwork/WATLAS/residence_patches/2021/islandica/batch1/underlying_data/"), full.names=T)
# files_2022 <- list.files(path =paste0("//zeus/cos/birds/bijleveld/fieldwork/WATLAS/residence_patches/2022/islandica/batch1/underlying_data/"), full.names=T)
...
#choose residence patches that are not roosting (<50NAP), adults only, not our birds that were in captivity and not 2 other birds that were identified as acting strange. 
...
p1
# ggplot(data= duration_df)+
#     geom_boxplot(aes(y = duration_min, fill=tod_start),outlier.size=0.5, col = "black", width = 0.25)+
#     scale_fill_manual(values=c("orange","grey15"))+
#     facet_grid(~inout)+
#     theme_bw()
...
plot(effect("tide_tod*low_level_scaled", model_furth))
...
# Predict
furth_df <- unique(bird_tides[,c("tide_tod", "tag", "year", "tideID","low_level_scaled")]
                    )
...
hist(bird_tides$furthest_patch)
...
# ggplot(data = furth_df, aes(x=tide_tod, y = furthest_patch, fill = tide_tod))+
...
model_area <- lmer(data=area_ptide, area_ptide ~ tide_tod + (1|year/id))
...
overlap <- st_read("C:/Users/BESCBEAR/OneDrive - Liverpool John Moores University/Synced files/Projects/1_NIOZ projects/2_DayNight/data/2018-2022_bird_tides_multipolygons_min5tides.gpkg") %>% 
...
setDT(overlap) #pivot wider no longer working with geom/sf

overlap<- as.data.frame(dcast(overlap, year+id  ~ tide_tod, value.var = c("geom", "num_tides"))) %>% 
...
summary_df <- read.csv("C:/Users/BESCBEAR/OneDrive - Liverpool John Moores University/Synced files/Projects/1_NIOZ projects/2_DayNight/data/2018-2022_postSept_SiteFidelityData.csv")
...
lplot <- p1 + theme(legend.position="top", legend.title = element_blank())
legend <- get_plot_component(lplot, "guide-box-top", return_all=T)
...
ggsave("C:/Users/BESCBEAR/OneDrive - Liverpool John Moores University/Synced files/Projects/1_NIOZ projects/2_DayNight/figures/Fig_3.png", plot_all,units="cm", height=40, width =40, dpi = 600)

cairo_ps("C:/Users/BESCBEAR/OneDrive - Liverpool John Moores University/Synced files/Projects/1_NIOZ projects/2_DayNight/figures/Fig_3.eps",  width = 15, height =15, pointsize=12, fallback_resolution = 1200)
print(plot_all)
dev.off()
...
# For supplementary Fig S2, look at different thresholds for site fidelity
...
S2 <- ggplot()+
        geom_boxplot(data= newdf2, aes(x= revisits, y = revisits_prob, fill = tide_tod), col="black", outlier.shape= 19)+
...
ggsave("C:/Users/BESCBEAR/OneDrive - Liverpool John Moores University/Synced files/Projects/1_NIOZ projects/2_DayNight/figures/Fig_S2.png", S2,units="in", height=7, width =7, dpi = 600)

cairo_ps("C:/Users/BESCBEAR/OneDrive - Liverpool John Moores University/Synced files/Projects/1_NIOZ projects/2_DayNight/figures/Fig_S2.eps",  width = 15, height =15, pointsize=12, fallback_resolution = 1200)
print(S2)
dev.off()
